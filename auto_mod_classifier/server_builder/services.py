...
class ServerJavaService:
    ...
    def _build_huaweicloud_java_url(self, required_major: int, package_name: str) -> str:
        if "-windows-" in package_name.lower():
            return f"https://mirrors.huaweicloud.com/openjdk/openjdk-{required_major}_windows-x64_bin.zip"
        return f"https://mirrors.huaweicloud.com/openjdk/openjdk-{required_major}_windows-x64_bin.zip"
    ...
    def _download_java_runtime(self, required_major: int, output_root: Path) -> JavaRuntime:
        ...
        huaweicloud_url = self._build_huaweicloud_java_url(required_major, package["name"])
        download_urls = [huaweicloud_url, package["link"]]

        self.common.log_line(f"未找到匹配 Java，准备自动下载 Java {required_major}：{package['release_name']}")
        self.common.log_line(f"下载源：优先尝试华为云镜像，失败后回退到官方源")
    ...
    def ensure_java(self, client_dir: Path, game_root: Path, candidate: VersionCandidate, output_root: Path) -> JavaRuntime:
        ...
        matched_exact = [item for item in runtimes if item.major == required_major and (item.is_64bit or not require_64bit)]
        if matched_exact:
        ...
        if compatible:
            compatible_sorted = sorted(compatible, key=lambda item: item.major)
            selected = compatible_sorted[0]
            self.common.log_line(f"未找到 Java {required_major}，使用兼容的高版本 Java {selected.major}")
            return selected

        if self.common.runtime.auto_download_java:
            self.common.log_line(f"当前未找到兼容的 Java（需要 >= {required_major}），将尝试自动下载到输出目录。")
        ...
        if require_64bit and any(item.major >= required_major for item in runtimes):
            raise RuntimeError(
                f"Minecraft {candidate.minecraft_version} 需要 64 位 Java {required_major}（或更高版本），"
                f"但当前扫描到的兼容版本 Java 不满足 64 位要求。\n{self.format_java_runtime_list(runtimes)}"
            )

        raise RuntimeError(
            f"Minecraft {candidate.minecraft_version} 需要 Java {required_major}（或更高版本），"
            f"但当前未找到兼容的 java.exe。\n{self.format_java_runtime_list(runtimes)}"
        )
...
class ServerInstallService:
    ...
    def classify_mod_directory(self, mods_dir: Path) -> List[Dict[str, Any]]:
        # 生成服务端里的模组筛选，复用外面的筛选主链，而不是另写一套规则。
...
class ServerWorkflowService:
    """生成服务端总流程编排器。真正的“先做什么、后做什么”写在这里。"""
