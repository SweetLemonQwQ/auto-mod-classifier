...
SETTINGS_FILE_PATH = get_application_dir() / "auto_mod_classifier_settings.json"
...
class App(FluentWindow):
    ...
    def _build_pages(self) -> None:
        ...
        self.addSubInterface(self.server_page, FIF.COMMAND_PROMPT, "生成服务端")
    ...
    def start_server_task(self) -> None:
        ...
        if not source_path.exists():
            ...
            return
        if not self.validate_source_path(source_path, "生成服务端输入源"):
            return
    ...
    def _refresh_report_sections(self) -> None:
        for panel_key, section in self.report_sections.items():
            ...
            if panel_key == "mod":
                ...
                section.status_label.setText("尚未开始生成服务端")
