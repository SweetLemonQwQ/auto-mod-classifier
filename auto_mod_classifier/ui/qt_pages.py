...
class QtPageFactory:
    ...
    def build_home_page(self) -> HomePageBuild:
        ...
        # 开服状态卡片
        server_card, srv_gl = self._create_card("生成服务端")
    ...
    def build_report_page(self) -> ReportPageBuild:
        ...
        sv_card, sv_l = self._create_card("生成服务端结果")
        sv_sr = QHBoxLayout()
        sv_sr.setSpacing(SPACING_SM)
        sv_sd = StatusDot(sv_card)
        sv_st = StrongBodyLabel("尚未开始生成服务端", sv_card)
        apply_themed_style(
            sv_st,
            lambda: f"color: {qt_theme.TEXT_PRIMARY}; background: transparent; font-size: {FONT_SIZE_MD}px; font-weight: 600;",
        )
        ...
        sv_br.addWidget(srb)
        spb = PushButton("返回生成服务端", sv_card)
    ...
    def build_settings_page(self) -> SettingsPageBuild:
        ...
        self._add_control_row(s_l, "下载源", sv_dl, "模组筛选和生成服务端共用这一项下载源设置。")
