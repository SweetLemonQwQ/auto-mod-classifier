...
class BuildServerUseCase:
    ...
    def execute(
        ...
        # 生成服务端同样先走输入整理，这样后面才能轻松支持目录、mrpack、zip 等来源。
        emit("stage", {"stage_key": "scan", "detail": "正在准备生成服务端输入源"})
        emit("status", "正在准备生成服务端输入源…")
        emit("log", f"开始准备生成服务端输入源：{request.source_path}")
        try:
